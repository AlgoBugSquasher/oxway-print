...
export default function PdfPageSelector() {
  ...
  const processFile = async (uploadedFile: File) => {
    ...
    const supportedExtensions = new Set(["pdf", "png", "jpg", "jpeg", "docx"]);
    if (!extension || !supportedExtensions.has(extension)) {
      return setError("Unsupported file. Upload a PDF, PNG, JPG, JPEG, or DOCX file.");
    }
    if (uploadedFile.size > MAX_FILE_SIZE) return setError("This file is larger than the 25 MB kiosk limit.");
    setFile(uploadedFile);
    setIsLoading(true);
    setThumbnails([]);
    setPreviewUrl(uploadedFile.type.startsWith("image/") ? URL.createObjectURL(uploadedFile) : "");
    setIsPaid(false);
    setPrintSuccess(false);
    try {
      const formData = new FormData();
      formData.append("file", uploadedFile);
      const conversionResponse = await fetch("/api/convert-file", { method: "POST", body: formData });
      if (!conversionResponse.ok) {
        const conversionError = await conversionResponse.json().catch(() => null);
        throw new Error(conversionError?.error || "Unable to convert this file.");
      }
      const pdf = await pdfjsLib.getDocument({ data: await conversionResponse.arrayBuffer() }).promise;
